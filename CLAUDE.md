...
| Editable week-plan action types in Settings | `DB.settings.actionTypeList` + settings UI + toggle cycles all | ✅ |
...
| Proforma invoice module: draft→awaiting_customer→sent→approved→invoiced + سوپرادمین rollback, auto-number PF-YYYY-NNNN | proforma tab + /api/proforma | ✅ |
