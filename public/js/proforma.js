...
async function pfOpenInvoices() {
  try {
    ...
    var body = rows.length ? '<table style="width:100%;border-collapse:collapse;font-size:12px"><thead><tr style="background:#f8fafc"><th style="padding:8px;text-align:right">شماره</th><th style="padding:8px;text-align:right">مشتری</th><th style="padding:8px;text-align:right">تاریخ</th><th style="padding:8px;text-align:right">مبلغ</th><th style="padding:8px;text-align:right">وضعیت</th></tr></thead><tbody>' +
      rows.map(function(inv) { var action = inv.wms_status === 'delivered'
        ? '<button onclick="pfOpenPayment(\'' + inv.id + '\')" style="font:inherit;font-size:11px;border:1px solid #bae6fd;background:#eff6ff;color:#0369a1;border-radius:6px;padding:4px 7px;cursor:pointer">پرداخت</button>'
        : '<button onclick="pfOpenDelivery(\'' + inv.id + '\')" style="font:inherit;font-size:11px;border:1px solid #86efac;background:#f0fdf4;color:#15803d;border-radius:6px;padding:4px 7px;cursor:pointer">تحویل</button>';
        return '<tr style="border-top:1px solid #e2e8f0"><td style="padding:8px;font-family:monospace">' + esc(inv.invoice_no || inv.id) + '</td><td style="padding:8px">' + esc(inv.center_name || '—') + '</td><td style="padding:8px">' + esc(inv.jalali_date || '—') + '</td><td style="padding:8px">' + fmtNum(inv.total || 0) + ' ﷼</td><td style="padding:8px">' + esc(inv.status || 'issued') + '</td><td style="padding:8px;white-space:nowrap">' + action + '</td></tr>'; }).join('') +
