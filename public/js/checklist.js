...
function toggleCk(id){
  ...
  DB.checklist[key].items=items;saveChecklistApi(_ckDate,currentUser,DB.checklist[key]);renderChecklist();
}
function saveCkNote(v){var key=_ckDate+'_'+currentUser;if(!DB.checklist[key])DB.checklist[key]={items:[],note:''};DB.checklist[key].note=v;saveChecklistApi(_ckDate,currentUser,DB.checklist[key]);}
